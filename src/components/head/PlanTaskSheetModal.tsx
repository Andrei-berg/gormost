      ...
      >
        ...
        <div className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-2">
              {[[task1, setTask1], [task2, setTask2], [task3, setTask3]].map(([val, set], i) => (
                <div key={i} className="flex gap-2 items-start">
                  <span className="text-white/30 text-sm mt-2 shrink-0">{i + 1}.</span>
                  <textarea
                    value={val as string}
          ...
          <div>
            ...
            {allItems.length === 0
              ? <div className="text-[11px] text-white/25 italic">Нет пунктов — план пуст</div>
              : (
                <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
                  {allItems.map(item => (
                    <div key={item.num} className="flex gap-2 text-xs text-white/60">
                      <span className="text-white/30 shrink-0">{item.num}.</span>
                      <span>
                        {item.location && <span className="text-white/80">{item.location}: </span>}
                        {item.description}
                        {item.timeRange && <span className="text-cyan-400 ml-1">{item.timeRange}</span>}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <div>
            ...
            {loadingWorkers
              ? <div className="text-[11px] text-white/30 italic">Загрузка...</div>
              : workers.length === 0
                ? <div className="text-[11px] text-white/25 italic">Нет сотрудников в этой смене</div>
                : (
